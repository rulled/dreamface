...
(function() {
  ...
  let extensionSubmitArmed = false;

  window.addEventListener('DreamFaceExtensionSubmitArm', () => {
    extensionSubmitArmed = true;
  });
  ...
  async function addNoWatermarkToAvatarSubmit(args) {
    const [input, init] = args;
    let bodyText = typeof init?.body === 'string' ? init.body : null;
    let bodyKind = bodyText === null ? 'unsupported' : 'init-string';
    ...
    if (bodyText === null && input instanceof Request) {
      try {
        ...
        return { args, patched: false, reason: 'request-body-unreadable', bodyKind: 'request' };
      }
    }

    if (bodyText === null) {
      return { args, patched: false, reason: 'unsupported-body', bodyKind };
    }

    const body = tryParseJson(bodyText);
    const workType = body?.work_type || body?.workType || '';
    if (workType !== 'AVATAR_VIDEO') {
      return { args, patched: false, reason: 'not-avatar-video', workType, bodyKind };
    }
    if (body.no_water_mark === 1 || body.noWaterMark === 1) {
      return { args, patched: true, reason: 'already-present', workType, bodyKind };
    }

    const field = Object.hasOwn(body, 'work_type') ? 'no_water_mark' : 'noWaterMark';
    const nextBody = JSON.stringify({ ...body, [field]: 1 });
    return {
  ...
  window.fetch = async function(...args) {
    ...
    if (isSubmit) {
      const isExtensionSubmit = extensionSubmitArmed;
      extensionSubmitArmed = false;
      const watermarkPatch = await addNoWatermarkToAvatarSubmit(args);
      args = watermarkPatch.args;
      // сигнал что submit-fetch реально стартанул (клик на Generate дошёл до сети).
      // content_script использует его, чтобы отличить "кнопка мертва" от "submit долго".
      try {
        window.dispatchEvent(new CustomEvent('DreamFaceSubmitFetchStarted', {
          detail: {
            startedAt: Date.now(),
            url,
            watermarkPatched: watermarkPatch.patched,
            watermarkPatchReason: watermarkPatch.reason,
            workType: watermarkPatch.workType || '',
            bodyKind: watermarkPatch.bodyKind || '',
          },
        }));
      } catch {}
      if (isExtensionSubmit && !watermarkPatch.patched) {
        const error = `watermark patch failed: ${watermarkPatch.reason || 'unknown'}`;
        try {
          window.dispatchEvent(new CustomEvent('DreamFaceSubmitFetchFailed', {
            detail: { at: 'watermark-patch', error },
          }));
        } catch {}
    ...
    try {
      ...
      if (isSubmit) {
        try {
          window.dispatchEvent(new CustomEvent('DreamFaceSubmitFetchFailed', {
            detail: { at: 'network', error: String(err?.message || err) },
          }));
        } catch {}
      }
      throw err;
    ...
    if (isSubmit) {
      // http-ошибка ещё до того как эмит success-сигнал сможет прийти
      if (!response.ok) {
        try {
          window.dispatchEvent(new CustomEvent('DreamFaceSubmitFetchFailed', {
            detail: { at: 'http', status: response.status, statusText: response.statusText },
          }));
        } catch {}
      }
