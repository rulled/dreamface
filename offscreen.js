import { transformMp4 } from './media-transform.js';
...
let mediaTransformMutexChain = Promise.resolve();
function withMediaTransformMutex(taskFn) {
  const run = mediaTransformMutexChain.then(() => taskFn(), () => taskFn());
  mediaTransformMutexChain = run.catch(() => {});
  return run;
}
...
async function handleMuxOne(payload) {
  ...
  const cropLeftPx = Math.max(0, Math.floor(Number(payload?.cropLeftPx) || 0));
  ...
  try {
    ...
    if (cropLeftPx > 0) {
      ...
      const result = await withMediaTransformMutex(() => transformMp4(outBytes, {
        cropLeftPx,
        onProgress(progress) {
          if (progress === 1 || progress - lastLoggedProgress >= 0.1) {
            lastLoggedProgress = progress;
            console.log('[offscreen] physical crop progress', Math.round(progress * 100));
          }
        },
      }));
      outBytes = result.bytes;
      transformed = true;
      console.log('[offscreen] physical crop complete', {
        cropLeftPx,
    ...
    const blobUrl = URL.createObjectURL(blob);
    return { ok: true, blobUrl, bytes: outBytes.byteLength, chapters, transformed };
  } catch (err) {
...
async function restoreRunState() {
  ...
  if (['preparing', 'normalizing', 'ready', 'running', 'stopping'].includes(runState.phase)) {
    ...
    runState.finishedAt = new Date().toISOString();
    runState.statusText = remainingTasks > 0
